...
module.exports = {
  ...
  setupSwaggerExposition(definition, app) {
    const definitionFiles = app.config.swagger.swaggerDefinition
    if (Array.isArray(definitionFiles)) {
      definitionFiles.map((definitionFile, index) => {
        const filePath = path.basename(definitionFile)
        app.config.routes.push({
          method: ['GET'],
          path: `${app.config.swagger.swaggerDefinitionHttpPath}/${filePath}`,
          handler: (req, res) => {
            return res.send(definition[index])
          }
        })
      })
    }
    ...
    Object.keys(operations).forEach(path => {
      ...
      Object.keys(methods).forEach(method => {
        if (method !== CONTROLLER_KEY) {
          const security = method.security || baseSecurity
